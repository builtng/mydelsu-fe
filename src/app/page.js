...
// myDelsu · Main Landing Page (renders the Manna Draw view). Route: /
...
const WEEK_DEFAULT = { faculty: "FOS", facultyName: "Faculty of Science", session: "25/26", sessionLong: "2025/2026 session", nextFaculty: "Faculty of Arts" };
...
const TOOLS = [
  { name: "GPA Calculator", desc: "Work out your semester GPA in seconds.", href: "/tools", icon: "calc" },
  { name: "CGPA Tracker", desc: "Enter semester results to track your running CGPA.", href: "/tools", icon: "target" },
];
...
function FlapTile({ value }) {
  ...
  useEffect(() => { if (value !== prev) setK((x) => x + 1); }, [value]);
...
export default function MyDelsuHome() {
  const { user } = useAuth();
  const [menuOpen, setMenuOpen] = useState(false);
  const [searchOpen, setSearchOpen] = useState(false);
  const [mode, setMode] = useState("idle");
  const [vals, setVals] = useState(["2", "3", "8", "4", "0", "2"]);
  ...
  const [shareUrl, setShareUrl] = useState("https://mydelsu.com");
  ...
  // Load status, winners, and gratitudes
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const gratRes = await apiFetch("/gratitudes");
        if (gratRes.ok) {
          const gratData = await gratRes.json();
          setGratitudes(gratData.data || []);
        }

        // 3. Load past winners publicly
        const winRes = await apiFetch("/manna/past-winners");
        if (winRes.ok) {
          const winData = await winRes.json();
          setPastWinners(winData.data || []);
        }

        // 4. Load authenticated user's wins
        if (user) {
          const winsRes = await apiFetch("/profile/wins");
          if (winsRes.ok) {
        ...
        console.error("Failed to load Manna page data", err);
      }
    }
    loadData();
  }, [user]);

  const outstandingWin = useMemo(() => {
    return userWins.find(w => (w.payout_status === 'paid' || w.payout_status === 'approved') && !w.gratitude);
  }, [userWins]);

  async function submitGratitude() {
    if (!outstandingWin || submittingGratitude) return;
    if (gratitudeBody.trim().length < 10) {
      showToast("Gratitude message must be at least 10 characters.");
      return;
    }
    ...
    try {
      const res = await apiFetch("/gratitudes", {
        method: "POST",
        body: JSON.stringify({
          winner_id: outstandingWin.id,
          body: gratitudeBody
        })
      });
      const data = await res.json();
      if (res.ok) {
        showToast("Thank you! Your gratitude has been submitted for review.");
        setGratitudeBody("");
        // Re-fetch wins
        const winsRes = await apiFetch("/profile/wins");
        if (winsRes.ok) {
        ...
        showToast(data.message || "Failed to submit gratitude.");
      }
    } catch {
  ...
  // Show sponsor message once a day & client-only states initialization
  useEffect(() => {
    setVals(Array.from({ length: 6 }, rnd));
  ...
  const diff = now ? Math.max(0, nextFriday6pm(now) - now) : 0;
  ...
  async function entry() {
    ...
    try {
      const res = await apiFetch("/entries/confirm", { method: "POST" });
      const data = await res.json();
      if (res.ok) {
        setEntered(true);
        setEntrantsCount(prev => prev + 1);
        showToast("You are entered for this week. Come back on Monday to enter again.");
      } else {
        showToast(data.message || "Entry confirmation failed.");
      }
    } catch {
      showToast("Failed to connect to server. Try again shortly.");
    }
  }
  ...
  function copyLink() { try { navigator.clipboard.writeText(shareUrl); showToast("Link copied. Thank you for sharing."); } catch (e) { showToast("Share this link: " + shareUrl); } }
  const enc = encodeURIComponent;
  const waHref = "https://wa.me/?text=" + enc(SHARE_TEXT + " " + shareUrl);
  const fbHref = "https://www.facebook.com/sharer/sharer.php?u=" + enc(shareUrl);
  const xHref = "https://twitter.com/intent/tweet?text=" + enc(SHARE_TEXT) + "&url=" + enc(shareUrl);
  const tgHref = "https://t.me/share/url?url=" + enc(shareUrl) + "&text=" + enc(SHARE_TEXT);
  ...
  const perPage = 4;
  const listWinners = pastWinners.length > 0 ? pastWinners : [
    { user: { name: "Rita Ufuoma" }, draw: { cohort: { faculty: { code: "LAW" }, session_start: 2023 } }, date_label: "3 July 2026" },
    { user: { name: "David Akpobome" }, draw: { cohort: { faculty: { code: "FOA" }, session_start: 2024 } }, date_label: "26 June 2026" },
  ];
  
  const pageCount = Math.max(1, Math.ceil(listWinners.length / perPage));
  const shownWinners = listWinners.slice(page * perPage, page * perPage + perPage);
  const initials = (n) => n.split(" ").map((w) => w[0]).slice(0, 2).join("").toUpperCase();

  const activeFacultyName = activeCohort?.faculty_name || WEEK_DEFAULT.facultyName;
  const activeSessionLong = activeCohort ? `${activeCohort.session_start}/${activeCohort.session_start + 1} session` : WEEK_DEFAULT.sessionLong;
  const activeFacultyCode = activeCohort?.faculty_code || WEEK_DEFAULT.faculty;
  const activeSessionLabel = activeCohort ? `${activeCohort.session_start % 100}/${(activeCohort.session_start + 1) % 100}` : WEEK_DEFAULT.session;
  ...
  return (
    <div className="manna">
        ...
        <div className="modal-overlay" onClick={() => setAuthOpen(false)}>
          <div className="auth-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="am-actions">
              <a className="am-primary" style={{ textDecoration: "none" }} href="/register">Create account</a>
              <a className="am-ghost" style={{ textDecoration: "none" }} href="/login">Log in</a>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="modal-overlay" onClick={() => setProof(null)}>
          <div className="proof-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="pf-alert">
              ...
              <div className="pf-row"><span>To</span><b>{proof.user?.name || proof.name}</b></div>
              <div className="pf-row"><span>Account</span><b>******{proof.payout_reference ? proof.payout_reference.slice(-4) : "2261"}</b></div>
              <div className="pf-row"><span>Reference</span><b>{proof.payout_reference || "MNA-2261"}</b></div>
              <div className="pf-row"><span>Date</span><b>{proof.reviewed_at ? new Date(proof.reviewed_at).toLocaleDateString() : "3 July 2026"}, 6:04pm</b></div>
            </div>
      ...
      {/* Nav */}
      <nav className="nav">
        <a href="/" className="logo" style={{ textDecoration: "none" }}><span className="logo-my">my</span><span className="logo-badge"><span className="logo-badge-inner">DELSU</span></span></a>
      ...
      <main className="stage">
        ...
        <section className="draw-card">
          <div className="draw-head"><span className="eyebrow">This week&#39;s draw</span></div>
          <div className="tray">
            <p className="cohort">{activeFacultyCode} {activeSessionLabel}</p>
            <div className="flip-row" role="img" aria-label="Winning registration number">{vals.map((d, i) => (<FlapTile key={i} value={d} />))}</div>
            <p className="cohort-sub">{activeFacultyName} · {activeSessionLong}</p>
          </div>

          <div className="countdown"><span className="cd-label">Draw closes Friday at 6pm (Africa/Lagos)</span>
          ...
          <div className="winner-slot" aria-live="polite">
            {winner && (<div className="winner-card"><span className="winner-label">This week&#39;s blessing</span><span className="winner-num">{activeFacultyCode}/{activeSessionLabel}/{winner.reg}</span><span className="winner-sub">receives {prizeText}</span></div>)}
          </div>
        </section>

        {/* Gratitude submission form for winners */}
        {outstandingWin && (
          <section className="draw-card" style={{ marginTop: "20px", background: "#fffdf5", border: "1px solid #f0e2c6", textAlign: "left" }}>
            <span className="eyebrow" style={{ color: "var(--amber)", display: "block", marginBottom: "8px" }}>Congratulations! You won Manna!</span>
            <h2 style={{ fontSize: "20px", fontWeight: "800", margin: "0 0 6px" }}>Share Your Gratitude</h2>
            <p style={{ fontSize: "14px", color: "var(--body)", margin: "0 0 16px", lineHeight: "1.5" }}>
              Your payout has been verified and paid. As part of the myDelsu Manna covenant, please share a brief message of how this blessing helped you. Your words will encourage others on the Wall of Thanks!
            </p>
            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              <textarea
                value={gratitudeBody}
                onChange={(e) => setGratitudeBody(e.target.value)}
                placeholder="Write your thank you message here (minimum 10 characters)..."
                rows={4}
                style={{ width: "100%", padding: "12px", border: "1px solid var(--line)", borderRadius: "12px", fontSize: "14.5px", fontFamily: "inherit", outline: "none", resize: "vertical" }}
              />
              <button
                className="draw-btn"
                onClick={submitGratitude}
                disabled={submittingGratitude || gratitudeBody.trim().length < 10}
                style={{ alignSelf: "flex-start", marginTop: "4px" }}
              >
                {submittingGratitude ? "Submitting..." : "Submit to Wall of Thanks"}
              </button>
            </div>
          </section>
        )}
        ...
        <section className="stats">
          <div className="stat"><span className="stat-num">{NAIRA}{((pastWinners.length || 12) * PRIZE).toLocaleString()}</span><span className="stat-lab">Given away so far</span></div>
          <div className="stat"><span className="stat-num">{pastWinners.length || 12}</span><span className="stat-lab">Fridays held</span></div>
          <div className="stat"><span className="stat-num">{entrantsCount || 42}</span><span className="stat-lab">Weekly entries</span></div>
        </section>
        ...
        <section className="thanks">
          ...
          <div className="thanks-row">
            {(gratitudes.length > 0 ? gratitudes : [
              { user: { name: "Blessing Okpako" }, winner: { prize_kobo: 500000 }, body: "I honestly did not believe it until the alert came in. It paid for my handout and I still had a little left. I have promised myself to bless a junior next semester." },
              { user: { name: "Emmanuel Tega" }, winner: { prize_kobo: 500000 }, body: "Manna came through for me in a hard week. Thank you for keeping your word. God bless myDelsu." }
            ]).map((t, idx) => (
              <div className="thanks-card" key={idx}>
                <div className="tc-head"><span className="avatar">{initials(t.user?.name || "Anonymous")}</span><div><p className="tc-name">{t.user?.name}</p><p className="tc-fac">{t.user?.faculty || "Faculty of Science"}</p></div></div>
                <p className="tc-text">{t.body}</p>
                <button className="proof-btn" onClick={() => setProof({ name: t.user?.name, ref: "MNA-" + (2200 + idx), date: "Recently" })}><svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 11l3 3L22 4" /><path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11" /></svg>Proof</button>
              </div>
            ))}
          </div>
        ...
        <section className="winners">
          ...
          <ul className="winner-grid">
            {shownWinners.map((w, idx) => (
              <li className="winner-item" key={idx}>
                <div className="badge"><Laurel /></div>
                <div className="winner-body">
                  <p className="wi-name">{w.user?.name}</p>
                  <p className="wi-matric">{w.draw?.cohort ? `${w.draw.cohort.faculty?.code || "FOS"}/${w.draw.cohort.session_start}` : "LAW/23/24"}</p>
                  <p className="wi-faculty">{w.draw?.cohort?.faculty?.name || "Faculty of Science"} · {w.date_label || "Weekly Draw"}</p>
                </div>
                <span className="wi-amount">{prizeText}</span>
              </li>
            ))}
          </ul>
        ...
        <section className="sponsor-cta">
          <div className="sc-card">
            ...
            <a className="sc-btn" style={{ textDecoration: "none" }} href="/sponsor">Become a sponsor</a>
          </div>
        </section>
        ...
        <section className="tools">
          ...
          <div className="tools-row">
            {TOOLS.map((t) => (
              <a className="tool-card" href={t.href} key={t.name}>
          ...
          <div className="tools-more"><a className="see-all" href="/tools">See all student tools</a></div>
        </section>
...
function Wheat({ size = 22 }) { return (<svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={AMBER} strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22V9" /><path d="M12 9c-2-1.5-2-4 0-6 2 2 2 4.5 0 6Z" /><path d="M12 13c-2.5-1-3.5-3-3-5.5 2.4.6 3.7 2.4 3 5.5Z" /><path d="M12 13c-2.5-1-3.5-3-3-5.5-2.4.6-3.7 2.4-3 5.5Z" /><path d="M12 18c-2.5-1-3.5-3-3-5.5 2.4.6 3.7 2.4 3 5.5Z" /><path d="M12 18c2.5-1 3.5-3 3-5.5-2.4.6-3.7 2.4-3 5.5Z" /></svg>); }
...
.am-primary{ background:var(--blue); color:#fff; border:none; border-radius:11px; padding:12px; font-size:15px; font-weight:700; cursor:pointer; text-align:center; }
.am-primary:hover{ background:var(--blueDark); }
.am-ghost{ background:#fff; color:var(--ink); border:1px solid var(--line); border-radius:11px; padding:12px; font-size:15px; font-weight:700; cursor:pointer; text-align:center; }
...
.burger{ width:28px; height:22px; flex-direction:column; justify-content:space-between; background:transparent; border:none; }
...
.cd-chip{ font-family:ui-monospace, Menlo, Consolas, monospace; font-size:17px; font-weight:800; color:var(--ink); background:var(--soft); border:1px solid var(--line); border-radius:99px; padding:6px 9px; min-width:44px; }
...
.proof-btn{ align-self:flex-start; display:inline-flex; align-items:center; gap:6px; background:#eef4fb; color:var(--blueDark); border:1px solid #d9e6f3; border-radius:99px; padding:8px 14px; font-size:13px; font-weight:700; cursor:pointer; }
